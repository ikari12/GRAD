...
% ============================================================
Under speed-based DI, the data support a parsimonious two-dimensional durability structure rather than the proposed four-construct framework (CRQ1). PCA on standardized person-level medians confirmed this: the first eigenvalue ($1.61$) explained 53.6\% of variance, while the second ($1.01$) fell below the 95th-percentile parallel analysis threshold ($1.031$), justifying the retention of a single principal factor for durability and fatigability. Gradient correction (GACD) successfully isolates true cardiac drift from route geometry, with VIFs remaining low (median: gradient 2.01, speed 2.09, time 1.07), ensuring coefficient stability. We propose gradient sensitivity and speed sensitivity---the GACD regression coefficients---as route-corrected durability profiling metrics. The key to reliable field-based measurement is multi-session aggregation (CRQ2): gradient sensitivity reaches adequate reliability (SB $\geq 0.80$) at $\geq 5$ sessions, providing a concrete protocol for practitioners. The stable Person signal (22--44\%) is recoverable through aggregation; the transient Occasion component (residual variance) remains substantial (56--84\% in single sessions) but reflects day-to-day physiological variation that likely carries information about readiness and recovery. Route-matched ICC analysis showed that 18--31 percentage points of this apparent Occasion variance is actually route-related, further validating the necessity of GACD. Integrating multi-session gradient sensitivity with daily readiness markers represents a promising direction for robust field-based fitness monitoring.
