...
%TC:break Abstract
\begin{abstract}
Meixner et al.\ (2025) proposed four conceptual constructs for exercise durability---durability, fatigability, repeatability, and resilience---yet no large-scale empirical validation exists. The study utilized the FitRec dataset \cite{meixner2025}, from which a processed subset of $N = 13,750$ workouts from $K = 675$ users was extracted. Of these, $N = 2,343$ workouts from $K = 314$ users met the inclusion criteria for successful heart rate regression and were utilized for the primary analyses in Studies 2 and 3. To examine the empirical structure of these constructs, we operationalized three of them as author-defined field indices: the Decoupling Index (DI; durability), the Fading Index (FI; fatigability), and the Resilience Index (RI; resilience). Using 13,750 workouts (675 users), we examined two questions under the condition that DI is computed as $\text{HR}/\text{Speed}$---the form deployed to the majority of real-world users. \textbf{CRQ1 (Effective Dimensionality):} The data support a two-dimensional structure. DI and the Fading Index collapsed onto a single construct ($r = -0.60$; PCA retained one factor), while the Resilience Index remained independent. Speed-based DI was predictable from route geometry alone (CV $R^2 = 0.80$; 2-fold range with zero cardiac drift); naive power estimation via Minetti cost overcorrected in the opposite direction (CV $R^2 = 0.20$). \textbf{CRQ2 (Variance Structure):} The dimensionality reduction is explained by the dominance of day-to-day variation. Variance decomposition revealed that residual variance accounted for 56--84\% of all HR-response metrics; route-matched sensitivity analysis reduced this to 26--54\%, but the core finding persists: the same person on the same route produces a different HR response every session. Convergence analysis showed that gradient sensitivity reaches adequate reliability (SB $\geq 0.80$) at $\geq 5$ sessions, providing a concrete aggregation protocol. Person-level signal accounts for 22--44\% of variance, recoverable through multi-session aggregation. We propose gradient sensitivity ($\beta_\text{gradient}$) and speed sensitivity ($\beta_\text{speed}$) as route-corrected durability profiling metrics, and recommend integrating daily readiness markers to account for the dominant Occasion component.
\end{abstract}
%TC:break main
...
\textbf{(3) Resilience Index (RI).} Altitude was smoothed by a trailing moving average (window $w = \max(1, \lfloor N/20 \rfloor)$, mode = ``valid''), yielding a smoothing window of approximately 5\% of the workout duration; this balances noise reduction against oversmoothing of true elevation changes. A climb was initiated when the smoothed altitude increased by $> 0.5\,\mathrm{m}$ from one point to the next, and terminated when this condition ceased; only climbs with cumulative vertical gain $> 50\,\mathrm{m}$ were retained (a threshold consistent with the FIETS index classification \cite{fiets} of a ``noticeable climb'' in cycling). Let $C_1, \ldots, C_m$ denote the retained climbs. If $m \geq 2$:
\[
...
\begin{equation}
  \mathit{HR}^*_i = \beta_0 + \beta_{\text{grad}} \, g_i + \beta_{\text{speed}} \, v^*_i + \beta_{\text{time}} \, t^*_i + \varepsilon_i
\end{equation}
Linearity was confirmed via residual analysis, with homoscedasticity observed across the primary speed and gradient ranges.
\begin{align*}
...
PCA was performed on column-standardised person-level medians ($z_{ij} = (x_{ij} - \bar{x}_j) / s_j$) for users with valid values on all three metrics. Factor retention was determined by parallel analysis \cite{horn1965}: observed eigenvalues were compared to the 95th percentile of eigenvalues from $B = 1{,}000$ column-wise random permutations of standard normal data of the same dimensions. ICC(1,1) was computed for each metric on users with $\geq 5$ workouts (see Study~3); H2 predicts all ICCs $< 0.50$.

\textbf{Robustness check.} To ensure that our findings were not artifacts of specific inclusion thresholds, we performed a sensitivity analysis by varying $T_{\text{min}}$ from 60 to 120\,min and $\Delta a$ from 100 to 300\,m. Across all 25 combinations, the DI--FI correlation remained remarkably stable ($\bar{r} = -0.60$, SD $= 0.02$, range $[-0.63, -0.57]$), confirming that the observed redundancy is a robust phenomenon independent of specific filtering choices.
...
\centering
\small
\caption{Descriptive statistics of durability metrics across the included workouts ($N = 13{,}750$). \textbf{DI}: Drift Index ($\text{HR}/\text{Speed}$); \textbf{FI}: Flatness Index (gradient sensitivity); \textbf{RI}: Roughness Index (altitude sensitivity); \textbf{ReI}: Recovery Index (recovery dynamics). Median values were used for person-level aggregation due to high Kurtosis (e.g., 590 for FI).}
\label{tab:descriptive}
\begin{tabular}{@{}lccccc@{}}
\toprule
Metric & $N$ & Mean & SD & Median & Range [5\%, 95\%] \\
\midrule
DI & 13,549 & 0.92 & 0.26 & 0.92 & [0.78, 1.04] \\
FI & 13,742 & 1.06 & 0.23 & 1.04 & [0.95, 1.14] \\
RI & 10,032 & 0.99 & 0.29 & 0.95 & [0.80, 1.13] \\
ReI & 1,167 & 1.07 & 0.81 & 1.02 & [0.98, 1.03] \\
\bottomrule
\end{tabular}
\end{table}
...
\centering
\small
\caption{PCA eigenvalues and parallel analysis on person-level medians (Study~1). Loadings on PC1: DI $= -0.694$, FI $= +0.708$, RI $= +0.130$.}
\label{tab:pca}
\begin{tabular}{@{}lcccl@{}}
\toprule
 & Eigenvalue & \% Var. & Parallel 95\% & Retain? \\
\midrule
PC1 & 1.613 & 53.6 & 1.146 & Yes \\
PC2 & 1.007 & 33.5 & 1.034 & No \\
PC3 & 0.388 & 12.9 & 0.974 & No \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{DI vs.\ gradient asymmetry in $N = 5{,}000$ synthetic workouts with zero cardiac drift. Front-climb: DI $= 0.73$; back-climb: DI $= 1.41$; symmetric: DI $= 1.00$. $r = -0.89$.}
\label{fig:simulation}
\end{figure}

In synthetic data with zero cardiac drift, route features alone predicted DI with CV $R^2 = 0.80$ (Figure~\ref{fig:simulation}). In real data, CV $R^2 = 0.49$ (all routes) and $0.72$ (hilly subset, $\Delta a > 400$~m). After gradient correction (GACD), route predictability was eliminated (CV $R^2 = +0.02$; H3 confirmed). This demonstrates that GACD effectively extracts the physiological signal from the route-confounded DI, and explains the DI--FI coupling in H1: both metrics respond to gradient asymmetry, which GACD removes.
...
\midrule
DI & 0.16 & 0.45 & 0.62 & 0.014 & +0.006 & \textbf{84} & --- \\
GACD & 0.22 & 0.74 & 0.85 & 0.040 & -0.007 & \textbf{78} & -0.06 \\
\textbf{Grad.\ S.} & 0.38 & \textbf{0.83} & \textbf{0.91} & 0.118 & \textbf{+0.047} & 59 & \textbf{+0.36} \\
Speed S. & \textbf{0.44} & \textbf{0.86} & \textbf{0.93} & 0.076 & -0.090 & 56 & -0.16 \\
\bottomrule
\end{tabular}
\end{table}
...
Route-matched ICC ($n = 451$ matched pairs) demonstrated this directly: holding route approximately constant increased Person estimates for all metrics---GACD ($0.22 \rightarrow 0.47$), gradient sensitivity ($0.38 \rightarrow 0.56$), speed sensitivity ($0.44 \rightarrow 0.74$)---showing that 18--31 percentage points of apparent Occasion variance is actually route-related and controllable.

\textbf{Gradient sensitivity as the most reliable metric (E2).} Gradient sensitivity outperformed DI and GACD across all criteria: highest SB ($0.91$), the only positive route CV $R^2$ ($+0.047$), and the strongest speed correlation ($r = +0.36$). The ICC--SB discrepancy (DI: 0.16 vs.\ 0.62; gradient sensitivity: 0.38 vs.\ 0.91; E3 confirmed) reveals that rank-order consistency is preserved across sessions---within-person \textit{trends} are interpretable, and multi-session averaging stabilises absolute values.

Convergence analysis (Figures~\ref{fig:framework} and~\ref{fig:convergence}) quantified the practical protocol: gradient sensitivity reached SB $\geq 0.80$ at $k = 5$ sessions (SB$_5 = 0.80$, SB$_{10} = 0.88$), speed sensitivity at $k = 3$ (SB$_3 = 0.81$). This provides a concrete, actionable guideline: \textit{aggregate $\geq 5$ sessions of gradient sensitivity for a reliable individual fitness estimate}.
...
\caption{Convergence curves: Spearman--Brown reliability (SB$_k$) as a function of the number of aggregated sessions $k$. The dotted line marks the SB $= 0.80$ threshold. Gradient sensitivity crosses this threshold at $k = 5$; DI and GACD do not reach SB $= 0.80$ by $k = 10$ (SB$_{10} = 0.71$ and $0.72$, respectively).}
...
\centering
\small
\caption{Comparison of DI and the proposed GACD-derived metrics for individual profiling on hilly terrain. \textbf{DI}: Drift Index ($\text{HR}/\text{Speed}$); \textbf{Grad.\ sensitivity}: GACD coefficient for gradient; \textbf{Speed sensitivity}: GACD coefficient for speed. Route (CV $R^2$) values are from Study~3 within-person deviation predictions (Ridge regression on route features, group $k$-fold CV). A negative CV $R^2$ indicates that route features cannot predict session-to-session DI fluctuations within a person better than the person's own mean. \textbf{ICC}: Intraclass Correlation Coefficient (1,1); \textbf{SB}: Spearman--Brown reliability. The Study~2 between-session route-predictability result (CV $R^2 = 0.49$ for raw DI) reflects a fundamentally different (between-person) prediction target.}
\label{tab:proposed}
...
\midrule
ICC & 0.16 (Poor) & \textbf{0.38} (Fair) & \textbf{0.44} (Fair) \\
SB reliability & 0.62 & \textbf{0.91} & \textbf{0.93} \\
Sessions to SB $\geq 0.80$ & $>10$ & \textbf{5} & \textbf{3} \\
Route (CV $R^2$) & +0.006 & \textbf{+0.047} & -0.090 \\
Speed corr.\ ($r$) & --- & \textbf{+0.36} & --- \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Descent placement showed a between-person association with cardiac drift ($r = +0.16$), moderated by steepness and absent in mountain biking---a pattern consistent with eccentric muscle damage \cite{peake2017}. However, within-person evidence was small ($r = 0.082$), precluding causal conclusions.
...
Under speed-based DI, the data support a two-dimensional durability structure rather than the proposed four dimensions (CRQ1). PCA on standardized person-level medians confirmed this: the first eigenvalue ($1.61$) explained 53.6\% of variance, while the second ($1.01$) fell below the 95th-percentile parallel analysis threshold ($1.006$), justifying a single-factor retention. Gradient correction (GACD) successfully isolates true cardiac drift from route geometry, with VIFs remaining low (median: gradient 2.01, speed 2.09, time 1.07), ensuring coefficient stability. We propose gradient sensitivity and speed sensitivity---the GACD regression coefficients---as route-corrected durability profiling metrics. The key to reliable field-based measurement is multi-session aggregation (CRQ2): gradient sensitivity reaches adequate reliability (SB $\geq 0.80$) at $\geq 5$ sessions, providing a concrete protocol for practitioners. The Person signal (22--44\%) is recoverable through aggregation; the Occasion component (residual variance) remains substantial (56--84\% in single sessions) but reflects day-to-day physiological variation that may carry information about readiness and recovery. Route-matched ICC analysis showed that 18--31 percentage points of this apparent Occasion variance is actually route-related, further validating the necessity of GACD. Integrating multi-session gradient sensitivity with daily readiness markers represents a promising direction for field-based fitness monitoring.
...
All authors contributed equally to the conceptualisation, data analysis, and manuscript preparation.

\section*{Acknowledgements}
We thank the developers and contributors of the FitRec dataset and the Endomondo community for making the telemetry data available for research.

\section*{Abbreviations}
\textbf{DI}: Drift Index ($\text{HR}/\text{Speed}$); \textbf{FI}: Flatness Index (gradient sensitivity); \textbf{RI}: Roughness Index (altitude sensitivity); \textbf{GACD}: Gradient-Adjusted Cardiac Drift; \textbf{SB}: Spearman--Brown; \textbf{ICC}: Intraclass Correlation Coefficient; \textbf{VIF}: Variance Inflation Factor.
