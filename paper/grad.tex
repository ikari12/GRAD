...
%TC:break Abstract
\begin{abstract}
Millions of athletes use the speed-based Decoupling Index (DI) displayed by GPS-watch platforms, yet its sensitivity to route geometry has never been quantified. This study analysed $N = 13{,}750$ real-world workouts from $K = 675$ users to examine whether route terrain---rather than physiological cardiac drift---dominates speed-based DI, and to test the empirical dimensionality of the four-construct durability framework proposed by Meixner et al.\ (2025). Three speed-based indices were defined: DI (durability), the Fading Index (FI; fatigability), and the Resilience Index (RI; resilience). A controlled simulation ($N = 5{,}000$ synthetic workouts with zero cardiac drift) showed that route geometry alone produces DI values spanning 0.70--1.40 (cross-validated $R^2 = 0.80$); in real data, route features predicted DI with $R^2 = 0.49$, eliminated after Gradient-Adjusted Cardiac Drift (GACD) regression ($R^2 \approx 0.02$). Notably, 42\% of routes were front-climb profiles that mechanically produce DI $< 1.0$ even absent cardiac drift, suggesting widespread misinterpretation on hilly terrain. PCA on person-level medians provided only fragile support for a two-construct organisation (DI and FI redundant, $r = -0.60$; RI independent but unreliable, ICC $= 0.10$). Variance decomposition revealed that Occasion variance dominates all metrics (56--84\%), necessitating multi-session aggregation: gradient sensitivity reached adequate reliability (Spearman--Brown $\geq 0.80$) at $k \geq 5$ sessions, confirmed for cycling-specific data (Occasion 50--78\%); the running subsample was too small for stable estimation. Speed-based DI is heavily contaminated by route geometry; gradient-adjusted, multi-session profiling provides a more reliable signal for longitudinal durability monitoring.
\end{abstract}
%TC:break main
