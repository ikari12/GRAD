...
\]
where $\mathcal{B}^* = \{b : |\mathcal{S}_1 \cap \mathcal{B}_b| > 3 \text{ and } |\mathcal{S}_2 \cap \mathcal{B}_b| > 3\}$. Of the $N = 13{,}395$ workouts for which FI was computed, 99.9\% ($N = 13{,}387$) had at least one valid gradient bin in $\mathcal{B}^*$; FI was undefined for the remaining 8 workouts.
...
\begin{table}[ht]
\centering
\caption{\textbf{Supplementary Table~S1.} Sport-specific variance decomposition for the high-engagement subset. Cycling ($K = 94$ users with $\geq 5$ sessions, $N = 1{,}647$ workouts) shows the Occasion-dominant pattern consistent with pooled results. Mountain biking ($K = 10$, $N = 172$) shows even higher Occasion dominance, though the small sample limits generalisability. Running ($N = 47$ workouts, only 3 users with $\geq 5$ sessions) was insufficient for stable ICC estimation and is omitted.}
