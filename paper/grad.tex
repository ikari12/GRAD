...
\end{align*}
where $\mathbf{X} \in \mathbb{R}^{|\mathcal{V}| \times 4}$ (intercept, $g$, $v^*$, $t^*$) yielded: (4)~GACD ($\hat{\beta}_{\text{time}}$, bpm/min); (5)~Gradient sensitivity ($\hat{\beta}_{\text{grad}}$, bpm/\%); (6)~Speed sensitivity ($\hat{\beta}_{\text{speed}}$, bpm/(m/s)). Per-workout $R^2 = 1 - \|\mathbf{y} - \mathbf{X}\hat{\boldsymbol{\beta}}\|^2 / \|\mathbf{y} - \bar{y}\mathbf{1}\|^2$. Although gradient ($g_i$) and speed ($v^*_i$) exhibit substantial collinearity in field environments (athletes slow on ascents and accelerate on descents), the large number of data points within each filtered workout ($T_j > 90$~min; median $|\mathcal{V}| \approx 800$) provides sufficient degrees of freedom to yield stable coefficient estimates despite elevated variance inflation factors.
...
\]
\begin{align*}
  \text{MS}_{\text{B}} &= \frac{\sum_{k=1}^{K} n_k (\bar{x}_{k\cdot} - \bar{x}_{\cdot\cdot})^2}{K-1}, \quad
  \text{MS}_{\text{W}} = \frac{\sum_{k=1}^{K} \sum_{j=1}^{n_k} (x_{kj} - \bar{x}_{k\cdot})^2}{N-K}, \\[4pt]
...
The three studies form a constructive progression. Study~1 established that a two-dimensional structure best fits the data (H1), with DI and FI capturing a single gradient-response factor and RI providing an independent dimension. Study~2 showed that regression-based gradient correction (GACD) successfully isolates true cardiac drift from route geometry ($R^2 = 0.82 \rightarrow -0.03$; H3), confirming Millet's \cite{millet2025} theoretical insight with quantitative evidence. Study~3 demonstrated that multi-session aggregation overcomes the dominant day-to-day variability (57--83\%; H4), with gradient sensitivity reaching adequate reliability at $\geq 6$ sessions (SB $\geq 0.80$). Friel's $< 5\%$ rule \cite{friel2009}, itself never empirically validated, is applicable only on flat or symmetric courses; gradient correction extends DI's validity to all terrains.
...
\begin{tabular}{@{}lccc@{}}
\toprule
Attribute & DI (cardiac drift) & Gradient sensitivity & Speed sensitivity \\
\midrule
...
Route artifact (CV $R^2$) & $-2.30$ & \textbf{+0.089} & +0.003 \\
Speed correlation ($r$) & --- & \textbf{+0.33} & --- \\
\bottomrule
\end{tabular}
\end{table}
