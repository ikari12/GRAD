...
DI and FI showed $r = -0.60$ (95\% CI $[-0.69, -0.49]$; bootstrap $B = 2{,}000$ on person-level medians), confirming H1: DI and FI capture the same underlying gradient-response coupling, supporting a more parsimonious two-dimensional model. PCA on standardized person-level medians retained one factor (eigenvalue$_1 = 1.61$, 53.6\%; eigenvalue$_2 = 1.01$, below the parallel-analysis 95th-percentile threshold; Table~\ref{tab:pca}). RI was largely independent (DI--RI: $r = -0.19$; FI--RI: $r = +0.19$), confirming a two-dimensional structure: one DI--FI factor and one RI factor (Figure~\ref{fig:difi}).
...
\centering
\small
\caption{PCA eigenvalues and parallel analysis on person-level medians. Loadings on PC1: DI $= -0.694$, FI $= +0.708$, RI $= +0.130$.}
\label{tab:pca}
\begin{tabular}{@{}lcccl@{}}
\toprule
