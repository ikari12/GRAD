...
Aerobic decoupling（心拍:ペース比の前後半比較）は持久力の標準的フィールド指標として広く使用されているが，起伏コースでの妥当性は検証されていない．本研究は FitRec データセット（253,020 ワークアウト，675 ユーザー）を用いて，Decoupling Index（DI）の構成概念妥当性，ルート依存性，および分散構造を検証した．シミュレーション（N = 5,000，cardiac drift = 0）により，DI はルート形状のみから R² = 0.82 で予測可能であること — すなわち生理的情報を含まない数学的アーティファクトであること — を証明した．実データでも起伏コースで R² = 0.60 を確認した．このアーティファクトを除去すると，下りの時間的配置が心拍ドリフトを増加させる真の生理的効果（Cohen's d = 0.69）が明らかになった．Person / Route / Occasion の3軸による分散分解では，全 HR 応答指標で当日変動（Occasion）が 50–78% を占め，努力度では説明不能であった（r = −0.004 ns）．代替指標として提案する勾配感受性は，Spearman-Brown 信頼性 0.90（5 回で収束），唯一の正の交差検証 R²（+0.089），および平均速度との有意な相関（r = +0.33）を示した．起伏コースでは DI を持久力評価に使用すべきでなく，ルート補正と当日コンディション（readiness）情報の統合が不可欠である．
...
### D1. Aerobic Decoupling は起伏コースで測定不能である（C1）
...
> Aerobic decoupling は起伏コースで持久力の妥当な指標ではない．ルート予測 R² = 0.82（シミュレーション）/ 0.60（実データ）は，DI 値の大部分が計算式の地形非対称性への感応であることを証明する．このアーティファクトの下に，下りの時間的配置が心拍ドリフトを増加させる真の生理的効果（d = 0.69）が隠れていた．HR 応答分散の 78% は当日変動が占め，「同じ人が同じルートを走っても毎回違う」ことが定量的に示された．勾配感受性（SB = 0.90，5 回で収束，速度相関 r = +0.33）が代替候補として最も有望であるが，ICC = 0.36 が示すように単独での個人間比較には不十分であり，readiness 情報との統合が不可欠である．
